...
interface PeerContactDao {
    ...
    @Query("SELECT * FROM peer_contacts WHERE displayName = :name LIMIT 1")
    suspend fun getPeerByName(name: String): PeerContact?

    @Query("SELECT SUM(totalGiven - totalReceived) FROM peer_contacts")
    ...
    @Query("DELETE FROM peer_contacts WHERE id = :id")
